...
public class UserService {
    ...
    @Transactional
    public void requestDeletion(String auth0Id) throws Auth0Exception {
        User user = getUserByAuth0IdOrThrow(auth0Id, true);

        if (user.getDeletionRequestedAt() != null) {
            log.warn("El usuario '{}' ya tiene una solicitud de eliminación pendiente", user.getEmail());
            return;
        }

        LocalDateTime now = LocalDateTime.now();
        LocalDateTime scheduledDeletion = now.plusDays(30);

        user.setDeletionRequestedAt(now);
        user.setScheduledDeletionAt(scheduledDeletion);

        log.info("Solicitud de eliminación registrada para el usuario '{}'. Se eliminará el {}",
                user.getEmail(), scheduledDeletion);
    }

    @Transactional
    public UserResponse cancelDeletion(String auth0Id) {
        User user = getUserByAuth0IdOrThrow(auth0Id, true);

        if (user.getDeletionRequestedAt() == null) {
            log.debug("El usuario '{}' no tiene una solicitud de eliminación pendiente", user.getEmail());
            return userMapper.toResponse(user);
        }
        ...
        log.info("Solicitud de eliminación cancelada para el usuario '{}'", user.getEmail());
    ...
    public void executeScheduledDeletions() {
        ...
        for (User user : usersToDelete) {
            try {
                ...
                log.error("Error eliminando usuario '{}' (ID: {}). Se continuará con los siguientes",
                        user.getEmail(), user.getId(), e);
                // Opcional: marcar usuario para retry o notificar administradores
            }
        }
    }
    ...
    protected void deleteSingleUser(User user) throws Auth0Exception {
        ...
        log.info("Usuario '{}' eliminado exitosamente de Auth0 y BD", user.getEmail());
    }
